...
export class E2ePage {
    ...
    constructor(page) {
        this.page = page;
        this.topTitlePDP = page.getByTestId("topbarTitle");
        this.buttonCart = page.getByTestId("topbarCart");
        this.skeletonPDP = page.getByTestId("productDetailFinalPriceSkeleton");
        this.buttonATCPDP = page.getByTestId("productDetailAtcTrigger");
        this.buttonATCSelectFormat = page.getByTestId("productDetailAtcSelectedTrigger");
        ...
        this.variantGroup = page.getByTestId('productDetailAtcSelectionDrawerContent').getByTestId('productDetailVariantTriggerButton')
        ...
        this.applyFormat = page.getByTestId('productDetailAtcSelectionApply');
        this.buttonATCDrawer = page.getByTestId('productDetailAtcCta');
        this.selectWarehouse = page.getByRole('radiogroup').getByText('Gramedia Jakarta Matraman');
        ...
        this.address = page.locator(".mt-2").nth(0);
        this.skeletonAddress = page.locator('.mt-6 > .animate-pulse')
        ...
        this.pickUpInStore = page.locator('button > .truncate', { hasText: 'Ambil di Toko' });
        this.servicePIS = page.getByRole('radiogroup').locator('.truncate', { hasText: 'Gramedia Jakarta Matraman' });
        this.ninjaXpress = page.locator('button > .truncate', { hasText: 'Ninja Xpress' });
        ...
        this.paymentMethod = page.getByRole('button', { name: 'Pilih Metode Pembayaran' });
        ...
        this.savePayment = page.locator(".px-4 > .relative", { hasText: "Simpan" });
        ...
        this.toggleSummary = page.getByTestId('arrowToggleSummary');

        this.promoLabel = page.getByTestId('promoInputLabel')
    ...
    async selectEbook() {
        await this.buttonATCSelectFormat.click();
        await this.chipEbook.click();
        await this.applyFormat.click();
    }
    ...
    async clickAddToCart() {
        await this.topTitlePDP.click();
        await this.buttonATCPDP.click();
    }

    async clickATCDrawer() {
        await this.buttonATCDrawer.click();
    }
    ...
    async selectJNE() {
        await this.JNE.waitFor({ state: "attached" });
  ...
  async selectServiceJNERegular() {
    ...
    async inputOtpCC() {
        await this.iframeOtp.locator('input[type="password"]').fill(process.env.OTP_CODE_CC, { timeout: 10000 });
        await this.iframeOtp.getByRole('button', { name: 'OK' }).click();
    }

    async insertVoucher() {
        await this.promoLabel.click();
    }
    ...
    async validateSubTotalPrice() {
        ...
        await this.toggleSummary.click();
        ...
        const totalDiscount = parseInt(
            (await this.totalDiscount.innerText()).replace(/Rp|\.|,/g, "")
        ) * -1;
        ...
        const sumTotalPrice = totalBillPrice + totalShippingPrice - totalDiscount - voucherDiscount;
        await expect(subTotalPrice).toEqual(sumTotalPrice);
    ...
    async validateSubTotalTYP() {
        await this.buttonShopAgain.waitFor({ state: "visible" , timeout: 60000 });
        await this.buttonOrderList.waitFor({ state: "visible" , timeout: 60000 });
        ...
        const subTotalTYP = parseInt((await this.subTotalTYP.innerText()).replace(/Rp|\.|,/g, ""));
        await expect(subTotalTYP).toEqual(sumTotalPrice);
    ...
    async clickCartButton() {
      ...
      async validateEmptyCart() {
        await expect(this.titleCartEmpty).toBeVisible({ timeout: 10000 });
      }
}